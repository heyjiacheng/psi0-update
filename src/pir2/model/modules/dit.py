...
class TimestepEncoder(nn.Module):
    ...
    def forward(self, timesteps):
        # timesteps: (B,) → returns (B, D); (B, S) → returns (B, S, D)
        dtype = next(self.parameters()).dtype
        if timesteps.dim() == 2:
            B, S = timesteps.shape
            proj = self.time_proj(timesteps.flatten()).to(dtype)
            return self.timestep_embedder(proj).view(B, S, -1)
...
class AdaLayerNorm(nn.Module):
    ...
    def forward(
        ...
        # temb: (B, D) → broadcast scale/shift across tokens, or (B, S, D) → per-token.
...
class DiT(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        # Output processing — auto-detect 2D vs 3D temb (matches AdaLayerNorm).
        conditioning = temb
