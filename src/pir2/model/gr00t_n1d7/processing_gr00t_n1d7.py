...
from pir2.configs.data.embodiment_configs import ModalityConfig
from pir2.data.embodiment_tags import EmbodimentTag
from pir2.data.interfaces import BaseProcessor
from pir2.data.state_action.state_action_processor import StateActionProcessor
from pir2.data.utils import parse_modality_configs, to_json_serializable
...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def __init__(
        ...
        # Async-VLM training augmentation. Stored for backwards-compat with
        # setup.py but no longer actively used here: the d-sampling lives in
        # extract_step_data (sharded_single_step_dataset.py), which shifts the
        # video index by -d and attaches d to VLAStepData.image_delay. This
        # processor just reads content.image_delay and forwards it into the
        # batch. The model side gates delay_embedding on its own config flag.
        ...
        # Async-VLM augmentation.
        self.image_delay_max = int(image_delay_max)
    ...
    def process_state_only(
        self,
        state_data: dict[str, np.ndarray],
        embodiment_tag: EmbodimentTag,
    ) -> dict[str, Any]:
        """State-only fast path for the decoupled cached inference loop.

        Mirrors the state branch of ``__call__`` — normalize via
        StateActionProcessor, concat across modality keys, pad to
        ``max_state_dim``, cast to default dtype — and skips image transforms
        and tokenization entirely. Output is a dict that ``action_head.get_action``
        consumes directly (it needs only ``state`` and ``embodiment_id``).

        Saves ~3-4 ms / call on the cached path vs running the full processor.
        """
        ...
        return {
            ...
            "embodiment_id": self.embodiment_id_mapping[embodiment_tag.value],
        }
    ...
    def __call__(
        ...
        # Async-VLM image-delay augmentation. The dataset's extract_step_data has
        # already sampled d and loaded a single frame at position step_index - d
        # (see sharded_single_step_dataset.py); the value of d is on content.image_delay.
        # The processor just forwards it into the batch so the action head's
        # delay_embedding can condition on it. No slicing needed here.
        image_delay_value = int(getattr(content, "image_delay", 0))
        ...
        # Async-VLM: per-sample image_delay scalar (long). Always present so the
        # collator stacks into a (B,) tensor consistently; the action head's
        # delay_embedding only reads it when self.config.image_delay_embed_dim > 0
        # (= delay_embedding module exists). Value is the sampled d for training,
        # 0 for eval / non-augmented case.
        transformed_inputs["image_delay"] = torch.tensor(image_delay_value, dtype=torch.long)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | Path, **kwargs):
        ...
        if kwargs:
            ...
            override_keys = [
                ...
                "model_name",
                "model_type",
                # Local addition: allow training to override these when the
                # finetune action_horizon / state_dim / action_dim differs from
                # the base checkpoint's saved values.
                "max_action_horizon",
                "max_state_dim",
                "max_action_dim",
            ]
