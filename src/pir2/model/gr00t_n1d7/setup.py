...
from pir2.configs.base_config import Config
from pir2.configs.model.gr00t_n1d7 import Gr00tN1d7Config
from pir2.data.dataset.factory import DatasetFactory
from pir2.experiment.dist_utils import get_rank
from pir2.model.base.model_pipeline import ModelPipeline
from pir2.model.gr00t_n1d7.gr00t_n1d7 import Gr00tN1d7
from pir2.model.gr00t_n1d7.processing_gr00t_n1d7 import Gr00tN1d7Processor
from pir2.model.registry import register_model
    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            model, loading_info = AutoModel.from_pretrained(
                ...
                # Action chunk length + streaming knobs (local additions). Base checkpoint's
                # config.json doesn't have these keys, so they'd silently default if not
                # forwarded here. HF's from_pretrained passes named kwargs through to the
                # config object, overriding any disk values.
                action_horizon=self.config.model.action_horizon,
                streaming=self.config.model.streaming,
                streaming_num_chunks=self.config.model.streaming_num_chunks,
                streaming_constant_weight=self.config.model.streaming_constant_weight,
                streaming_linear_weight=self.config.model.streaming_linear_weight,
                streaming_random_weight=self.config.model.streaming_random_weight,
                streaming_chunk_wise_weight=self.config.model.streaming_chunk_wise_weight,
                streaming_symmetric_j=self.config.model.streaming_symmetric_j,
                streaming_j_range_max=self.config.model.streaming_j_range_max,
                streaming_inference_j_shift=self.config.model.streaming_inference_j_shift,
                streaming_mask_clean_end=self.config.model.streaming_mask_clean_end,
                streaming_chunk_size_max=self.config.model.streaming_chunk_size_max,
                streaming_rtc_weight=self.config.model.streaming_rtc_weight,
                streaming_rtc_d_max=self.config.model.streaming_rtc_d_max,
                streaming_schedule_mode=getattr(self.config.model, "streaming_schedule_mode", "pir2"),
                image_delay_max=self.config.model.image_delay_max,
                image_delay_embed_dim=self.config.model.image_delay_embed_dim,
            ...
            # delay_embedding is a new module (async-VLM training augmentation)
            # that may not exist in older base ckpts. It's zero-initialized in
            # the model constructor (gr00t_n1d7.py:103), so missing-from-ckpt is
            # benign — the zero init means the model behaves identically to a
            # no-delay-embedding setup at training start, then learns the embedding.
            if any("delay_embedding" in key for key in missing_keys):
                logging.info("delay_embedding not in checkpoint - zero-initialized")

            unexpected_keys = loading_info.get("unexpected_keys", [])
            mismatched_keys = loading_info.get("mismatched_keys", [])
            other_missing = [
                k for k in missing_keys
                if "mask_token" not in k and "delay_embedding" not in k
            ]
            errors = []
            if other_missing:
    ...
    def _create_dataset(self, save_cfg_dir: Path):
        ...
        if self.config.training.start_from_checkpoint is not None:
            processor = AutoProcessor.from_pretrained(
                ...
                # Async-VLM training augmentation.
                image_delay_max=self.model_config.image_delay_max,
                **self.transformers_loading_kwargs,
            )
        else:
            processor = self.processor_class(
                ...
                state_dropout_prob=self.model_config.state_dropout_prob,
                use_mean_std=self.model_config.use_mean_std,
                # Async-VLM training augmentation (image-delay sampling). When
                # > 0 the processor samples d ~ U[0, image_delay_max] per sample
                # and slices video to keep only the frame at offset -d, writing
                # image_delay=d to the batch.
                image_delay_max=self.model_config.image_delay_max,
                transformers_loading_kwargs=self.transformers_loading_kwargs,
            )
