...
from pir2.data.types import ModalityConfig
...
class PolicyServer:
    ...
    def __init__(
        ...
        # Decoupled VLM/DiT endpoints (only present on DecoupledGr00tPolicy).
        if hasattr(self.policy, "update_vlm_cache"):
            self.register_endpoint("update_vlm_cache", self.policy.update_vlm_cache)
        if hasattr(self.policy, "get_action_chunk_cached"):
            self.register_endpoint(
                "get_action_chunk_cached", self.policy.get_action_chunk_cached
            )
        if hasattr(self.policy, "reset_streaming_buffer"):
            self.register_endpoint(
                "reset_streaming_buffer",
                self.policy.reset_streaming_buffer,
                requires_input=False,
            )
        if hasattr(self.policy, "seed_streaming_from_obs"):
            self.register_endpoint(
                "seed_streaming_from_obs", self.policy.seed_streaming_from_obs,
            )
