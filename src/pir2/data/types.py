...
class VLAStepData:
    ...
    # Async-VLM training augmentation: image staleness d in control ticks. Set by
    # extract_step_data when GR00T_IMAGE_DELAY_MAX env var is > 0 (training only).
    # Loader has already shifted video indices by -d; processor just forwards this
    # value into the batch as "image_delay" for the model's delay_embedding to read.
    image_delay: int = 0
