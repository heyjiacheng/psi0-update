...
from pir2.data.interfaces import ShardedDataset
from pir2.data.types import EmbodimentTag, MessageType, ModalityConfig, VLAStepData
...
def extract_step_data(
    ...
    # Async-VLM training augmentation: sample image delay d once per step. Loader
    # shifts video indices by -d (single-frame load at a randomly delayed position)
    # instead of loading the full [-D_MAX..0] window and slicing later. Result:
    # identical model input as the old design but with 1x memory pressure instead of
    # D_MAX+1 x, so BS=768 fits on 8x A5000 (vs OOM at BS>=512 before).
    # PIR2_IMAGE_DELAY_MAX is the name used in this tree; GR00T_IMAGE_DELAY_MAX is the
    # upstream pi-r2-flow name, accepted so upstream launch recipes work unchanged.
    image_delay_max = int(
        os.environ.get("PIR2_IMAGE_DELAY_MAX")
        or os.environ.get("GR00T_IMAGE_DELAY_MAX", "0")
    )
    image_delay = (
        int(np.random.randint(0, image_delay_max + 1)) if image_delay_max > 0 else 0
    )

    # Extract data for each configured modality
    for modality, config in modality_configs.items():
        step_data[modality] = {}
        # Sample timesteps according to delta indices configuration. For video,
        # shift step_index by -image_delay so the loaded frame is at position i-d.
        step_index_for_modality = (
            step_index - image_delay if modality == "video" else step_index
        )
        indices_to_load = [
            step_index_for_modality + delta_index for delta_index in config.delta_indices
        ]
        if allow_padding:
            indices_to_load = [max(0, min(idx, len(episode_data) - 1)) for idx in indices_to_load]
        elif modality == "video" and image_delay > 0:
            # Clamp video indices >= 0 for steps near the start where step_index - d
            # would be negative. Capping at 0 (= first frame) is the natural choice.
