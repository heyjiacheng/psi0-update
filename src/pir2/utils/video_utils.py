...
def get_frames_by_indices(
    ...
    # Pre-decoded npy cache shortcut: scripts/prepare_video_cache.py writes
    # <video_path>.npy (replacing .mp4) with uint8 (T,H,W,3) frames. When present,
    # bypass ffmpeg/torchcodec/decord and serve via mmap'd np.load — eliminates
    # the per-batch video decode bottleneck, especially with image_delay_max>0.
    import os as _os
    _cache_path = video_path[:-4] + ".npy" if video_path.endswith(".mp4") else None
    if _cache_path is not None and _os.path.exists(_cache_path):
        _arr = np.load(_cache_path, mmap_mode="r")
        return np.ascontiguousarray(_arr[list(indices)])
