...
from pir2.policy.gr00t_policy import Gr00tPolicy
from pir2.policy.replay_policy import ReplayPolicy
from pir2.policy.server_client import PolicyServer
import tyro
...
class ServerConfig:
    ...
    """Expose decoupled VLM/DiT endpoints (update_vlm_cache + get_action_chunk_cached).
    The legacy get_action endpoint still works. Has no effect with --dataset-path."""
...
def main(config: ServerConfig):
    ...
    if config.model_path is not None:
        ...
        policy_cls = DecoupledGr00tPolicy if config.decoupled else Gr00tPolicy
        policy = policy_cls(
        ...
        if config.decoupled:
            print("  Decoupled VLM/DiT endpoints enabled")
    ...
    if config.use_sim_policy_wrapper:
        from pir2.policy.gr00t_policy import Gr00tSimPolicyWrapper
